...
function Membership() {
  ...
  return (
    <section className="section">
          ...
          {PLANS.map((plan) => {
            ...
            return (
              <div className={`plan-card ${plan.featured ? 'is-featured' : ''}`} key={plan.name}>
                {plan.featured && !isCurrent && <span className="plan-card__ribbon">Most popular</span>}
                {isCurrent && <span className="plan-card__ribbon plan-card__ribbon--active">Your plan</span>}
                <h3>{plan.name}</h3>
                ...
                >
                  {isCurrent ? 'Current plan' : currentPlan ? `Switch to ${plan.name}` : `Choose ${plan.name}`}
                </button>
              </div>
            )
          })}
      ...
      {applyingPlan && (
        <MembershipApplication plan={applyingPlan} onClose={() => setApplyingPlan(null)} />
      )}
    </section>
  )
}
